...
int main() {
	...
	int master_listen, ack = 1, start_port, finish_port;
	char prename[15] = {0};
	char ip[20] = {0};
	char hostname[20] = {0};
	char temp_to[5] = {0};
	char temp_from[5] = {0};
	get_conf_value(config, "From", ip);
	get_conf_value(config, "PortTo", temp_to);
	get_conf_value(config, "PortFrom", temp_from);

    start_port = atoi(temp_from);
    finish_port = atoi(temp_to);
	 ...
	 */
	DBG("%s", "初始化队列     开始！\n");
	for (int i = start_port; i <= finish_port; ++i) {
		//printf("_______________________________________________%s.%d", prename, i);
		//printf("*****************************%s\n\n", hostname);
		initaddr.sin_addr.s_addr = inet_addr(ip);
		initaddr.sin_port = htons(i);
